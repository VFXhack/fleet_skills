...
**Amended by:** ADR 0020 (+ ADR 0021 rename) — adds `sequences`, `sequence_look_runs`,
`sequence_look_bindings` tables and a third `assets.scope` value (`sequence`) + `assets.sequence_code`
(migration `0004`). The 7-table count below is now 10 (+ the Sequence config/Look). Current live truth:
`CONTEXT.md` + migration `0004_sequences_and_look.sql`.
